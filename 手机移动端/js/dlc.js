...
function upLoadImg(file, picbox, size) {
	...
	for(var i = 0; i < file.files.length; i++) {
		...
		if(ext != ".BMP" && ext != ".PNG" && ext != ".GIF" && ext != ".JPG" && ext != ".JPEG") {
			...
			img_html = "<div class='isImg' style='height: 100%'><img src='" + objUrl + "' onclick='javascript:lookBigImg(this)' style='height: 100%; width: 100%;border-radius: .1rem;' /><button class='removeBtn'  style='position:absolute;right: 0;top: 0;background: rgba(0,0,0,0.2);color: #fff;border-radius: 50%;width: 0.3rem;height: 0.3rem;font-size: 0.1rem;display: flex;align-items: center;justify-content: center;'>x</button></div>";
...
//加载动画
function load(str) {
	var str = str ? str : '正在加载...';
	$('.load').remove();
	var str = '<div class="load" style="overflow: hidden;position:fixed;height: 1rem;width: 1.2rem;line-height: 1rem;top: 0;bottom: 0;left: 0;right: 0;margin: auto;background: #000;opacity: .4;color: #fff;border-radius: .05rem;text-align:center;z-index:50;"><i class="loading_pic" style="background:url(../img/loading.png) no-repeat center center;background-size:cover;display:inline-block;height:.26rem;width:.26rem;margin-right:.1rem;position:absolute;left:.0;right:0;top:.15rem;margin:auto;"></i><span style="display:inline-block;z-index:1000;margin-top:.15rem">' + str + '</span></div>';
	$('body').append(str);
}
...
//域名
function dlcUrl() {
	return 'http://plstcd.app.xiaozhuschool.com';
}
//弹框
function _alert(str) {
	var str = str ? str : '弹框提示';
	$('._alert').remove();
	var str = '<div class="_alert" style="overflow: hidden;position:fixed;height: .5rem;width: 2rem;line-height: .5rem;top: 0;bottom: 0;left: 0;right: 0;margin: auto;background: #000;opacity: .4;color: #fff;border-radius: .05rem;text-align:center;z-index:50;"><span style="display:inline-block;z-index:1000;">' + str + '</span></div>';
	$('body').append(str);
...
function user_openid(openid) {
	return localStorage.getItem('openid');
}
if(!user_token() && getUrlParam('code') == '') {
	location.replace("https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxc72b57ad73571b5c&redirect_uri=" + window.location.href + "&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect");
}
if(!user_token() && getUrlParam('code')) {
	dlc_request('', {
		...
		if(res.code == 1) {
			...
			set_openid(res.data.thirdinfo.openid);
			localStorage.setItem('avatar', res.data.userinfo.avatar); //头像
			localStorage.setItem('nickname', res.data.userinfo.nickname); //昵称
			localStorage.setItem('mobile', res.data.userinfo.mobile); //昵称
			//location.reload();
		}
	}, false);
}
